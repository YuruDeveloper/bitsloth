...
from bitsloth.utils import attention_dispatch as attention_dispatch_utils
from bitsloth.utils.packing import (
...
def _build_packed_training_setup(tmp_path, device):
    ...
    from bitsloth.models import llama as llama_mod
...
def test_enable_sample_packing():
    ...
    assert getattr(model, "_bitsloth_allow_packed_overlength") is True
    assert getattr(model.child, "_bitsloth_allow_packed_overlength") is True

    collator = trainer.data_collator
    assert collator.return_position_ids is True
    assert getattr(collator, "_bitsloth_packing_wrapped") is True
...
def test_enable_padding_free_metadata():
    ...
    collator = trainer.data_collator
    assert collator.return_position_ids is True
    assert getattr(collator, "_bitsloth_padding_free_lengths_wrapped") is True
