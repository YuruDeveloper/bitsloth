...
- `test_bitsloth_qlora_train_and_merge.py`: Test Bitsloth QLoRA train and merge using `FastLanguageModel.from_pretrained`, `FastLanguageModel.get_peft_model`, and `FastLanguageModel.save_pretrained_merged` apis
...
Run bitsloth test:
```bash
python tests/qlora/test_bitsloth_qlora_train_and_merge.py
```
...
For the bitsloth test, the model's behavior is as expected: 
