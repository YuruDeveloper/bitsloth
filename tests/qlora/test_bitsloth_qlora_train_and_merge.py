...
def get_bitsloth_model_and_tokenizer(
    model_name: str,
    max_seq_length: int,
    load_in_4bit: bool,
    fast_inference: bool,
...
if __name__ == "__main__":
    ...
    bitsloth_merged_path = "bitsloth_merged_16bit"

    model, tokenizer = get_bitsloth_model_and_tokenizer(
        model_name,
        max_seq_length=512,
        load_in_4bit=True,
    ...
    model.save_pretrained_merged(
        bitsloth_merged_path,
        tokenizer,
        save_method="merged_16bit",
    )
    merged_model_bitsloth, tokenizer = get_bitsloth_model_and_tokenizer(
        bitsloth_merged_path,
        max_seq_length=512,
        load_in_4bit=False,
        fast_inference=False,
    ...
    responses = sample_responses(
        merged_model_bitsloth,
        tokenizer,
        prompt=prompt,
        **generation_args,
    )
    with header_footer_context("Responses after bitsloth merge to 16bit"):
        check_responses(responses, answer=ANSWER, prompt=prompt)
