...
os.environ["BITSLOTH_IGNORED_TOKENIZER_NAMES"] = "\n".join(IGNORED_TOKENIZER_NAMES)
...
def convert_to_fast_tokenizer(
    slow_tokenizer,
    temporary_location = "_bitsloth_sentencepiece_temp",
):
...
def assert_same_tokenization(slow_tokenizer, fast_tokenizer):
    ...
    try:
        ...
        # For eg see https://github.com/bitslothai/bitsloth/issues/292
    ...
    try:
        ...
        try:
            ...
            if protobuf_version > Version("3.20.3"):
                raise RuntimeError(
                    f"Bitsloth: Your protobuf version = {protobuf_version} is too new.\n"
...
def fix_sentencepiece_gguf(saved_location):
    ...
    logger.warning(
        f"Bitsloth: Extending {saved_location}/tokenizer.model with added_tokens.json.\n"
...
def _load_correct_tokenizer(
    ...
    try:
        ...
        slow_tokenizer = None
        # print(
        #     f"Bitsloth: {tokenizer_name} has no tokenizer.model file.\n"\
    ...
    # Ignore Phi-4 ones as well
        ...
        if assert_same_tokenization(slow_tokenizer, fast_tokenizer):
            ...
            logger.warning(
                f"Bitsloth: Will load {tokenizer_name} as a legacy tokenizer."
            )
...
def load_correct_tokenizer(
    ...
    # Also check Llama-2 old style models
        ...
        if old_chat_template is not None and chat_template is None:
            raise RuntimeError(
                "Bitsloth: Fixing chat template failed - please file a report immediately!"
            )
        pass
    ...
    if no == yes:
        ...
        if (
            ...
            if (
                ...
                raise RuntimeError(
                    f"Bitsloth: The tokenizer `{tokenizer.name_or_path}`\n"
                    "does not have a {% if add_generation_prompt %} for generation purposes.\n"
                    f"Please file a bug report to the maintainers of `{tokenizer.name_or_path}` - thanks!"
                )
            else:
                logger.warning_once(
                    "Bitsloth: We successfully patched the tokenizer to add a {% if add_generation_prompt %} to the chat_template.\n"
    ...
    for j, index in enumerate(added_tokens_fast.keys()):
        if index >= max_embedding_size:
            ...
            if not _reload:
                ...
                if can_be_removed:
                    ...
                    if max(tokenizer.added_tokens_decoder.keys()) < max_embedding_size:
                        logger.warning_once(
                            f"Bitsloth loaded a broken tokenizer `{model_name}`, but managed to repair it!\n"
                ...
                # :( Failure
                raise RuntimeError(
                    f"Bitsloth tried to load `{model_name}`, but cannot succeed.\n"
                    f"Tokens {bad_tokens} with ids {bad_indices} exceeds the max vocab size of {max_embedding_size}.\n"
            ...
            try:
                ...
                logger.warning_once(
                    "Bitsloth: Tokenizer is most likely buggy, and Bitsloth failed to repair it.\n"
...
def get_tokenizer_info(tokenizer) -> dict:
    ...
    or displaying in the Bitsloth Studio UI. All fields are safe to access —
...
def patch_sft_trainer_tokenizer():
    ...
    for path_to_trainer in (
        ...
        check_text = (
            ...
            "        raise RuntimeError('Bitsloth: We do not support AMD / Intel machines yet - it is a work in progress!')\n"
