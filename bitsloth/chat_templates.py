...
# =========================================== Bitsloth
# Bitsloth efficient template leverages from Zephyr
bitsloth_template = \
    "{{ bos_token }}"\
...
bitsloth_ollama = _ollama_template("bitsloth")

bitsloth_eos_token = "eos_token"
CHAT_TEMPLATES["bitsloth"] = (bitsloth_template, bitsloth_eos_token, False, bitsloth_ollama,)
DEFAULT_SYSTEM_MESSAGE["bitsloth"] = "You are a helpful assistant to the user"
...
def _change_system_message(template: str, type_chat_template: str, system_message: str = None):
    ...
    if default_system_message is None:
        if system_message is not None:
            logger.warning_once(
                f"Bitsloth: You tried to change the system message for {type_chat_template}, "
    ...
    if type_chat_template is None:
        ...
        if has_placeholder:
            if system_message is None:
                raise ValueError("Bitsloth: You need to provide a system message for custom templates.")
...
def get_chat_template(
    ...
    if type(chat_template) in (list, tuple,):
        ...
        if not is_fast_tokenizer:
            pass
            # print(
            #     "Bitsloth: Not a fast tokenizer, so can't process it as of yet :(\n"\
            ...
            if map_eos_token and (not stop_word in token_mapping.values()):
                ...
                logger.warning_once(f"Bitsloth: Will map {stop_word} to EOS = {tokenizer.eos_token}.")
        ...
        raise TypeError(
            f"Bitsloth: `chat_template` must be a tuple of (your_template, eos_token,) or one of\n"\
...
def _parse_combined_prompt(combined_prompt, dataset):
    ...
    for column in possible_columns:
        if column not in dataset_columns:
            raise KeyError(
                f"Bitsloth: Your prompt includes '{column}' but this does not exist in the dataset. "\
...
def _create_formatter(possible_columns, final_optional_prompts, user_column_name):
    ...
    for j, optional_prompt in enumerate(final_optional_prompts):
        ...
        if len(needed_columns) == 0:
            raise IndexError("Bitsloth: Optional [[...]] blocks must contain at least 1 {column}.")
...
def to_sharegpt(
    ...
    if "conversations" in dataset.column_names:
        ...
        if type(convo) is list:
            raise TypeError("Bitsloth: Your dataset is probably already in ShareGPT format!")
    ...
    def __convert_to_sharegpt__(examples):
        ...
        if len(users) != len(assistants):
            raise ValueError(
                "Bitsloth: Input and output columns must have matching batch lengths. "
...
extra_eos_tokens = None,
    ...
    for extra_eos in extra_eos_tokens:
        ...
        if extra_eos not in vocab:
            raise ValueError(f"Bitsloth: `{extra_eos}` is not a singular token in the tokenizer.")

    error_msg = \
        "Bitsloth: Your prompt template must have 2 examples showing the user input {INPUT} "\
    ...
    if len(extra_eos_tokens) == 0:
        raise RuntimeError(
            "Bitsloth: Your tokenizer does not have an EOS token? Please provide one via extra_eos_tokens!"
        )
    ...
    try:
        ...
        if combined != final_combined_check:
            ...
            raise RuntimeError(
                "Bitsloth: The prompt template you provided isn't correct. You gave:\n"\
                f"{combined_changed}\n\n"\
    ...
    if count_eos == 0:
        logger.warning("Bitsloth: We automatically added an EOS token to stop endless generations.")
    ...
    if len(system_part) != 0:
        ...
        if "{SYSTEM}" in partial_system:
            if default_system_message is None:
                raise RuntimeError("Bitsloth: Please specify a default system message!")
...
extra_eos_tokens = None,
    ...
    tokenizer._bitsloth_input_part  = input_part
    tokenizer._bitsloth_output_part = output_part
    if hasattr(tokenizer, "tokenizer"):
        tokenizer.tokenizer.chat_template = jinja_template
        tokenizer.tokenizer._ollama_modelfile = modelfile
        tokenizer.tokenizer._bitsloth_input_part  = input_part
        tokenizer.tokenizer._bitsloth_output_part = output_part
...
def test_hf_gguf_equivalence(tokenizer, gguf_model = "./model-bitsloth.F16.gguf"):
    """
