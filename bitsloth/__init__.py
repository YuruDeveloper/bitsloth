...
# Log Bitsloth is being used
os.environ["BITSLOTH_IS_PRESENT"] = "1"
...
# This check is critical because Bitsloth optimizes these libraries by modifying
# their code at import time. If they're imported first, the original (slower,
# more memory-intensive) implementations will be used instead of Bitsloth's
...
if already_imported:
    ...
    warnings.warn(
        f"WARNING: Bitsloth should be imported before [{', '.join(already_imported)}] "
...
# Bitsloth currently does not work on multi GPU setups - sadly we are a 2 brother team so
...
if DEVICE_TYPE == "cuda":
    ...
    try:
        ...
        print(
            "Bitsloth: `bitsandbytes` is not installed - 4bit QLoRA unallowed, but 16bit and full finetuning works!"
        )
        bnb = None
    try:
        ...
        warnings.warn("Bitsloth: Running `ldconfig /usr/lib64-nvidia` to link CUDA.")
        ...
        try:
            ...
            warnings.warn(
                "Bitsloth: CUDA is not linked properly.\n"
                "Try running `python -m bitsandbytes` then `python -m xformers.info`\n"
                "We tried running `ldconfig /usr/lib64-nvidia` ourselves, but it didn't work.\n"
                "You need to run in your terminal `sudo ldconfig /usr/lib64-nvidia` yourself, then import Bitsloth.\n"
                "Also try `sudo ldconfig /usr/local/cuda-xx.x` - find the latest cuda version.\n"
                "Bitsloth will still run for now, but maybe it might crash - let's hope it works!"
            )
    del libcuda_dirs
elif DEVICE_TYPE == "hip":
