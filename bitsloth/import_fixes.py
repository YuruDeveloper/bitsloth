...
# We cannot do from bitsloth_zoo.log import logger since FBGEMM might cause seg faults.
...
def fix_message_factory_issue():
    try:
        ...
        if not hasattr(google.protobuf.message_factory, "MessageFactory"):
            logger.info(
                "Bitsloth: Patching protobuf.MessageFactory as it doesn't exist"
            )
            ...
            logger.info("Bitsloth: Patching protobuf.MessageFactory.GetPrototype")
        pass
    except:
        pass
            ...
            if cutlass.exists():
                with open(cutlass, "r+", encoding="utf-8") as f:
                    ...
                    if "num_splits_key=-1," in text:
                        ...
                        f.truncate()
                        logger.info(
                            "Bitsloth: Patching Xformers to fix some performance issues."
                        )
        except Exception as e:
            logger.info(f"Bitsloth: Failed patching Xformers with error = {str(e)}")
...
def patch_vllm_for_notebooks():
    ...
    logger.info(
        "Bitsloth: Notebook detected - Patching sys.stdout.fileno for newer `vllm>=0.12.0` versions"
    )
            ...
            if ovis_config.exists():
                ...
                backbone_config = AutoConfig.for_model(model_type,
                    ...
                    backbone_config = AIMv2Config(**backbone_config)""",
                        ...
                        f.truncate()
                        logger.info(
                            "Bitsloth: Patching vLLM to fix `'aimv2' is already used by a Transformers config, pick another name.`"
                        )
        except Exception as e:
            logger.info(f"Bitsloth: Failed patching vLLM with error = {str(e)}")
...
def patch_ipykernel_hf_xet():
    ...
    if (
        ...
        print(
            "#### Bitsloth: `hf_xet==1.1.10` and `ipykernel==7.0.0` or `ipykernel==7.0.1` breaks progress bars. Using ASCII progress bars.\n"
            "#### Bitsloth: To re-enable progress bars, please upgrade to `ipykernel>=7.1.0` or wait for a fix to\n"
...
def patch_datasets():
    ...
    if (datasets_version <= Version("4.5.0")) and (
        ...
        raise NotImplementedError(
            f"#### Bitsloth: Using `datasets = {str(datasets_version)}` will cause recursion errors.\n"
...
def check_fbgemm_gpu_version():
    ...
    if Version(fbgemm_gpu_version) < Version("1.4.0"):
        ...
        logger.info(
            f"Bitsloth: fbgemm_gpu_genai=={fbgemm_gpu_version} is old and may cause issues. "
            f"Disabling FBGEMM - using Triton kernels instead."
        )
        return

    logger.info(f"Bitsloth: fbgemm_gpu_genai=={fbgemm_gpu_version} detected.")
...
def patch_enable_input_require_grads():
    ...
    logger.info(
        "Bitsloth: Patched enable_input_require_grads for vision model compatibility"
    )
...
def torchvision_compatibility_check():
    ...
    if os.environ.get("BITSLOTH_SKIP_TORCHVISION_CHECK", "0").lower() in ("1", "true"):
        return

    if importlib.util.find_spec("torch") is None:
        raise ImportError("Bitsloth: torch not found. Please install torch first.")
    ...
    if tv_v >= Version(required_tv_str):
        logger.info(
            f"Bitsloth: torch=={torch_version_raw} and "
    ...
    message = (
        f"Bitsloth: torch=={torch_version_raw} requires "
    ...
    if is_custom or is_prerelease:
        ...
        logger.warning(
            ...
            f"Set BITSLOTH_SKIP_TORCHVISION_CHECK=1 to silence this."
        )
        return
    ...
    try:
        with open(openenv, "r+", encoding="utf-8") as f:
            ...
            if bad + "\n" + "\n" in text and replace_with not in text:
                ...
                f.truncate()
                logger.info(
                    "Bitsloth: Patching TRL OpenEnv to fix SamplingParams not defined"
                )
    except Exception as e:
        logger.info(f"Bitsloth: Failed patching TRL OpenEnv with error = {str(e)}")
    ...
    try:
        ...
        with open(executorch, "r+", encoding="utf-8") as f:
            ...
            if bad in text and what not in text:
                ...
                f.write(text)
                f.truncate()
                logger.info("Bitsloth: Patching Executorch to fix get_mapped_key")
    except Exception as e:
        logger.info(f"Bitsloth: Failed Executorch with error = {str(e)}")
...
def fix_huggingface_hub():
    ...
    if not hasattr(huggingface_hub, "is_offline_mode"):
        huggingface_hub.is_offline_mode = lambda: (
...
def fix_triton_compiled_kernel_missing_attrs():
    ...
    logger.info(
        "Bitsloth: Patched triton CompiledKernel with num_ctas/cluster_dims "
...
def patch_trunc_normal_precision_issue():
    ...
    if getattr(torch.nn.init, "_bitsloth_trunc_normal_patched", False):
        return

    original_trunc_normal = torch.nn.init.trunc_normal_
    if getattr(original_trunc_normal, "__bitsloth_trunc_normal_patched__", False):
        torch.nn.init._bitsloth_trunc_normal_patched = True
        return
    ...
    _patched_trunc_normal_.__bitsloth_trunc_normal_patched__ = True
    _patched_trunc_normal_._bitsloth_original = original_trunc_normal
    torch.nn.init._bitsloth_trunc_normal_original = original_trunc_normal
    torch.nn.init.trunc_normal_ = _patched_trunc_normal_
    torch.nn.init._bitsloth_trunc_normal_patched = True
    logger.info(
        "Bitsloth: Patched torch.nn.init.trunc_normal_ for fp16/bf16 stability."
    )
...
def check_vllm_torch_sm100_compatibility():
    ...
    raise RuntimeError(
        f"Bitsloth: Incompatible configuration detected.\n\n"
...
def fix_vllm_pdl_blackwell():
    ...
    try:
        ...
        if vllm_version >= Version(VLLM_PDL_FIX_VERSION):
            logger.info(
                f"Bitsloth: SM100 ({sm100_gpu_name}) detected but vLLM {vllm_version} "
                f"should include PDL fix - skipping workaround"
            )
            return
    except Exception as e:
        logger.debug(
            f"Bitsloth: vLLM version check failed ({e}), applying PDL workaround."
        )
    ...
    if patched:
        logger.info(
            f"Bitsloth: Applied PDL fix for SM100 ({sm100_gpu_name}) - "
            f"patched: {', '.join(patched)}"
        )
    else:
        # Just set the env var - vLLM might be an older version without supports_pdl
        logger.info(f"Bitsloth: Set TRITON_DISABLE_PDL=1 for SM100 ({sm100_gpu_name})")
...
def patch_openspiel_env_async():
    ...
    try:
        ...
        try:
            ...
            nest_asyncio.apply()
            logger.info(
                "Bitsloth: Applied nest_asyncio for OpenEnv EnvClient async compatibility"
            )
        except ImportError:
            logger.info(
                "Bitsloth: nest_asyncio not installed, OpenEnv async methods may need manual wrapping"
            )
...
def patch_torchcodec_audio_decoder():
    """Call bitsloth_zoo's AudioDecoder patch."""
    try:
        from bitsloth_zoo.dataset_utils import patch_torchcodec_audio_decoder as _patch
...
def disable_broken_wandb():
    ...
    try:
        ...
        logger.info(
            "Bitsloth: wandb is installed but broken (likely a protobuf version mismatch). "
...
_CAUSAL_CONV1D_BLOCKER_SENTINEL = "_bitsloth_causal_conv1d_blocker"
VLLM_BROKEN = False
_VLLM_PREFIX = "vllm"
_VLLM_BLOCKER_SENTINEL = "_bitsloth_vllm_blocker"
_ROCM_ENV_HINT_KEYS = (
    "ROCM_PATH",
    "ROCM_HOME",
    "HIP_PATH",
...
def _is_rocm_torch_build() -> bool:
    ...
    try:
        ...
        if "rocm" in torch_version_raw:
            _log_rocm_detection(
                "Bitsloth: ROCm detection matched torch version tag (+rocm)."
            )
            return True
    except Exception:
        pass
    ...
    for key in _ROCM_ENV_HINT_KEYS:
        ...
        if isinstance(value, str) and value.strip():
            _log_rocm_detection(
                f"Bitsloth: ROCm detection matched environment key `{key}`."
            )
            return True
    ...
    for path in _ROCM_PATH_HINTS:
        try:
            if path.exists():
                _log_rocm_detection(
                    f"Bitsloth: ROCm detection matched filesystem hint `{path}`."
                )
                return True
        except Exception:
            continue

    _log_rocm_detection("Bitsloth: ROCm detection did not match any known hints.")
    return False
    ...
    for candidate in _iter_amdgpu_asic_id_table_candidates():
        try:
            if candidate.is_file():
                ...
                if BITSLOTH_ENABLE_LOGGING:
                    logger.info(
                        f"Bitsloth: Set {_AMDGPU_ASIC_ID_TABLE_PATH_ENV}={candidate}"
                    )
...
def _get_vllm_cuda_mismatch_message(error):
    ...
    return (
        f"Bitsloth: vLLM was built for CUDA {wanted_cuda} but this system has "
...
def _patch_find_spec_for_causal_conv1d():
    ...
    if getattr(current_find_spec, "_bitsloth_causal_conv1d_find_spec_patch", False):
        return
    ...
    _blocked_find_spec._bitsloth_causal_conv1d_find_spec_patch = True
    _blocked_find_spec._bitsloth_original_find_spec = current_find_spec
    importlib.util.find_spec = _blocked_find_spec


def _patch_find_spec_for_vllm():
    current_find_spec = importlib.util.find_spec
    if getattr(current_find_spec, "_bitsloth_vllm_find_spec_patch", False):
        return
    ...
    _blocked_find_spec._bitsloth_vllm_find_spec_patch = True
    _blocked_find_spec._bitsloth_original_find_spec = current_find_spec
    importlib.util.find_spec = _blocked_find_spec
...
def disable_broken_vllm(error=None):
    ...
    if cuda_msg:
        ...
        logger.warning(
            "Bitsloth: Detected broken vLLM binary extension; "
            "disabling vLLM imports and continuing import.\n"
            "Please reinstall via `uv pip install bitsloth vllm torchvision torchaudio "
            "--torch-backend=auto`."
        )
    return True
...
def disable_broken_causal_conv1d():
    ...
    This mirrors Bitsloth's FlashAttention fallback behavior: if importing causal_conv1d
    ...
    _disable_transformers_causal_conv1d()
    print(
        "Bitsloth: Detected broken causal_conv1d binary; "
