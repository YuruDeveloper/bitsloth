...
def get_device_type():
    ...
    if hasattr(torch, "accelerator"):
        if not torch.accelerator.is_available():
            raise NotImplementedError(
                "Bitsloth cannot find any torch accelerator? You need a GPU."
            )
    ...
    raise NotImplementedError(
        "Bitsloth currently only works on NVIDIA, AMD and Intel GPUs."
    )
    ...
    if ALLOW_BITSANDBYTES:
        ...
        if Version(bitsandbytes.__version__) >= Version("0.49.2"):
            ...
            try:
                ...
                print(
                    "Bitsloth: Checking `from bitsandbytes.cextension import ROCM_WARP_SIZE_64` had error = \n"
                    f"{str(e)}\n"
