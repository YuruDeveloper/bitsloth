...
from bitsloth_zoo.rl_replacements import (
    RL_REPLACEMENTS,
    left_pack_padding,
...
from bitsloth_zoo.device_type import device_synchronize
...
def sft_trainer_fix_untrained_tokens(call_args, extra_args):
    if "model" in call_args and "train_dataset" in call_args:
        fix_tokenizer = (
            "IGNORED_TOKENIZER_NAMES = os.environ.get('BITSLOTH_IGNORED_TOKENIZER_NAMES', '').split('\\n')\n"
            "from bitsloth_zoo.tokenizer_utils import fix_untrained_tokens\n"
            "from bitsloth_zoo.training_utils  import fix_zero_training_loss\n"
...
def sft_trainer_prepare_dataset(function_name, function):
    ...
    check_text = (
        ...
        "    return dataset\n"
        "if 'tokenizer'          not in locals(): tokenizer = processing_class\n"
        "if 'formatting_func'    not in locals(): raise RuntimeError('Bitsloth: Please file a bug report - `formatting_func` does not exist!')\n"
...
def grpo_trainer__prepare_inputs(function_name, function):
    ...
    function = function.replace(
        ...
        "if os.environ.get('BITSLOTH_FORCE_FLOAT32', '0') == '0' else torch.float16):",
    )
    function = function.replace(
...
def grpo_trainer__generate_single_turn(function_name, function):
    ...
    # When Bitsloth fast inference LoRA is active, weights are already shared.
    ...
    def remove_sync_weights_block(match):
        ...
        return (
            f"{indent}# Bitsloth fast inference LoRA shares weights with vLLM already.\n"
...
def grpo_trainer__generate_and_score_completions(function_name, function):
    ...
    # We must adjust the vLLM sampling_logprob tensor in Bitsloth to account for this.
    ...
    _metadata_extraction = (
        "\n"
        "        # Bitsloth: Extract per-sample chat_template_kwargs before metadata is lost\n"
        ...
        "        self._bitsloth_batch_chat_kwargs = []\n"
        "        for _inp_ in inputs:\n"
        "            _kw_ = {}\n"
        ...
        "                        _kw_[_k_] = _inp_[_k_]\n"
        "            self._bitsloth_batch_chat_kwargs.append(_kw_)\n"
    )
...
def grpo_trainer_fix_maybe_apply_chat_template(function_name, function):
    ...
    replacement = """
        ...
        _batch_chat_kwargs_ = getattr(self, "_bitsloth_batch_chat_kwargs", None)
...
def grpo_trainer__get_per_token_logps(function_name, function):
    ...
    def _get_per_token_logps(
        ...
        if True:  # os.environ.get('BITSLOTH_USE_NEW_MODEL', '0') == '0':
            return None  # Bitsloth efficient GRPO
...
def grpo_trainer__get_per_token_logps_and_entropies(function_name, function):
    ...
    def _get_per_token_logps_and_entropies(
        ...
        **kwargs,
        ...
        # All Bitsloth code here in this function is licensed under AGPL3
        # if True: # os.environ.get('BITSLOTH_USE_NEW_MODEL', '0') == '0':
        #     return None, None  # logps, entropies Bitsloth efficient GRPO
        if compute_efficient:
            ...
            if self.args.bitsloth_grpo_mini_batch is None:
                B, multiplier = autotune_batch_and_chunks(
                    ...
                    vocab_dim,
                    dtype_bytes,
                    self.args.bitsloth_logit_chunk_multiplier,
                )
                B = total_rows // B
            else:
                B = self.args.bitsloth_grpo_mini_batch

                if self.args.bitsloth_logit_chunk_multiplier is None:
                    multiplier = max(4, seq_len // 4096)
                else:
                    multiplier = self.args.bitsloth_logit_chunk_multiplier
...
BitslothEfficientGRPO = RL_REPLACEMENTS["BitslothEfficientGRPO"]
grpo_accumulated_loss = RL_REPLACEMENTS["grpo_accumulated_loss"]
grpo_update_SamplingParams = RL_REPLACEMENTS["grpo_update_SamplingParams"]
RL_PRE_ITEMS["grpo_trainer"].append(inspect.getsource(grpo_compute_loss))
RL_PRE_ITEMS["grpo_trainer"].append(inspect.getsource(BitslothEfficientGRPO))
        ...
        if per_token_logps is not None:
            ...
            if hasattr(self.args, "loss_type"):
                (
                    ...
                    old_logps=old_logps,
                    ref_logps=ref_logps,
                    n_chunks=self.args.bitsloth_num_chunks,
                    loss_type=self.args.loss_type,
                    importance_sampling_level=self.importance_sampling_level,
                    epsilon_low=self.epsilon_low,
                    epsilon_high=self.epsilon_high,
...
# Fix KTO shape mismatch when Bitsloth model forward truncates input_ids
...
def kto_trainer_get_batch_logps(function_name, function):
    ...
    new = (
        "# Bitsloth: auto-truncate to shorter sequence length (model may have truncated input_ids)\n"
...
def openenv_vllm_reload_weights():
    ...
    # 1. Remove the reload_weights call (bitsloth handles weight reloading)
    # 2. Fix wake_up call to be compatible with bitsloth (remove tags to wake everything)
    #
    # The issue: TRL's wake_up(tags=["kv_cache"]) only wakes kv_cache, leaving is_sleeping=True
    # at the executor level. This causes bitsloth's patched generate to try waking up again,
    # resulting in double create_and_map on already-mapped handles.
    #
    # The fix: Use wake_up() with no tags, which wakes everything. Bitsloth's patched
    ...
    try:
        ...
        logger.info(f"Bitsloth: Failed to import trl openenv: {e}")
        logger.info(
            "Bitsloth: trl.experimental.openenv not available — skipping RL openenv patches."
        )
        return
    ...
    # Remove the reload_weights call - bitsloth handles this differently
    ...
    # This prevents double wake_up issues. Bitsloth's allocator skips weights anyway.
    src = re.sub(r"\.wake_up\(tags=\[.*?\]\)", ".wake_up()", src)

    if original_src == src:
        logger.warning("Bitsloth: Warning - regex did not match, patch may have failed")
        return

    # Execute and explicitly assign to module
    local_ns = {}
    exec(compile(src, "<bitsloth>", "exec"), openenv_utils.__dict__, local_ns)
    ...
    logger.info(f"Bitsloth: Patched trl openenv {patch_target_name}")
...
def vllm_generation_init_patch():
    ...
    try:
        ...
        logger.info(f"Bitsloth: Failed to import trl.generation.vllm_generation: {e}")
        return
    ...
    def patch_vllm_generation_method(method_name, transform, marker, filename_suffix):
        ...
        if method is None:
            logger.info(f"Bitsloth: Could not find VLLMGeneration.{method_name}")
            return False
        ...
        try:
            ...
            logger.info(
                f"Bitsloth: Could not get source of VLLMGeneration.{method_name}: {e}"
            )
            return False
        ...
        src = transform(src)
        filename = f"<bitsloth_trl_vllm_generation_{filename_suffix}_patch>"
    ...
    def patch_init_vllm(src):
        ...
        def replace_llm_block(match):
            ...
            return (
                ...
                f"{indent}    # Bitsloth already inits vLLM in fast inference mode. Do not redo :)\n"
                f"{indent}    self.llm = model.vllm_engine\n"
                f"{indent}    self.bitsloth_fast_inference_lora = True\n"
        ...
        if num_replacements == 0:
            raise RuntimeError(
                "Bitsloth: Warning - regex did not match, VLLMGeneration._init_vllm patch may have failed"
            )
        return patched_src
    ...
    def patch_sync_weights(src):
        ...
        def replace_sync_weights(match):
            ...
            guard = (
                "    if getattr(self, 'bitsloth_fast_inference_lora', False):\n"
                "        # Bitsloth fast inference LoRA shares weights with vLLM already.\n"
                "        return\n\n"
            )
        ...
        if num_replacements == 0:
            raise RuntimeError(
                "Bitsloth: Warning - regex did not match, VLLMGeneration.sync_weights patch may have failed"
            )
        return patched_src
    ...
    def patch_generate(src):
        ...
        if num_replacements == 0:
            raise RuntimeError(
                "Bitsloth: Warning - regex did not match, VLLMGeneration.generate patch may have failed"
            )
        return patched_src
    ...
    try:
        init_patched = patch_vllm_generation_method(
            "_init_vllm",
            patch_init_vllm,
            "self.bitsloth_fast_inference_lora = True",
            "init_vllm",
        )
    ...
    if init_patched:
        logger.info("Bitsloth: Patched trl VLLMGeneration._init_vllm")
    if sync_patched:
        logger.info("Bitsloth: Patched trl VLLMGeneration.sync_weights")
    if generate_patched:
        logger.info("Bitsloth: Patched trl VLLMGeneration.generate")
