...
from ._utils import *
from ._utils import apply_bitsloth_gradient_checkpointing
    ...
    if past_key_values is not None:
        ...
        # New since 4.56
            ...
            if hasattr(
                ...
                if not hasattr(base_model, "_bitsloth_mask_needs_device"):
                    ...
                    base_model._bitsloth_mask_needs_device = _check_needs_device(
                        base_model._prepare_4d_causal_attention_mask_with_cache_position
                    )
                ...
                if base_model._bitsloth_mask_needs_device:
...
def LlamaModel_fast_forward(
    ...
    **kwargs,
    ...
    if input_ids is not None and inputs_embeds is not None:
        raise ValueError(
            "Bitsloth: You cannot specify both decoder_input_ids and decoder_inputs_embeds at the same time"
        )
        ...
        raise ValueError(
            "Bitsloth: You have to specify either decoder_input_ids or decoder_inputs_embeds"
        )

    seq_length_with_past = seq_length

    # Fix out of bounds tokenization unless we were given packed metadata
    allow_overlength = getattr(self, "_bitsloth_allow_packed_overlength", False) or (
    ...
    if hasattr(self, "max_seq_length") and not allow_overlength:
        if seq_length > self.max_seq_length:
            ...
            logger.warning_once(
                f"Bitsloth: Input IDs of shape {shape} with length {seq_length} > the model's max sequence length of {self.max_seq_length}.\n"
    ...
    if past_key_values is None and self.training:
        ...
        # if use_cache:
        #     logger.warning_once(
        #         "Bitsloth: `use_cache=True` is incompatible with gradient checkpointing. Setting `use_cache=False`"
        #     )
        #     use_cache = False
    ...
    if (
        ...
        # bitsloth's check for granite too has "version >= 4.45.0 (rightly so)".
...
def CausalLM_fast_forward(fast_forward_inference):
    def _CausalLM_fast_forward(
        ...
        **kwargs,
        ...
        if os.environ.get("BITSLOTH_RETURN_HIDDEN_STATES", "0") == "1":
        ...
        if bsz == 1 and q_len == 1:
            ...
            RETURN_LOGITS = os.environ.get("BITSLOTH_RETURN_LOGITS", "0") == "1"
            # < 1024 Normal Bitsloth uses less VRAM!
            if bsz * q_len <= 1024 and not RETURN_LOGITS:
                # Use bitsloth_fused_ce_loss which actually calculates the best chunk size to reduce VRAM usage
                RETURN_LOGITS = False
    ...
    if hasattr(self, "config") and hasattr(self.config, "max_position_embeddings"):
        if (
            ...
            if hasattr(_ids, "shape") and (
                ...
                raise ValueError(
                    f"Bitsloth: input length {_ids.shape[-1]} + max_new_tokens {kwargs['max_new_tokens']} exceeds the maximum sequence length of {self.config.max_position_embeddings}!\n"
...
class FastLlamaModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        os.environ["BITSLOTH_USE_NEW_MODEL"] = "0"
        if trust_remote_code:
            if fast_inference:
                raise NotImplementedError(
                    "Bitsloth: Fast inference does not support `trust_remote_code` yet."
                )
            print(
                "Bitsloth: WARNING `trust_remote_code` is True.\n"
                "Are you certain you want to do remote code execution?"
            )
        if fast_inference:
            if not is_vLLM_available():
                print("Bitsloth: vLLM is not installed! Will use Bitsloth inference!")
                fast_inference = False
            if DEVICE_TYPE == "cuda":
                major_version, minor_version = torch.cuda.get_device_capability()
                if major_version < 7:
                    print(
                        "Bitsloth: vLLM does not work on older GPUs - will switch to Bitsloth inference!"
                    )
                    fast_inference = False
            elif DEVICE_TYPE == "hip":
                fast_inference = True
            if (
                bitsloth_vllm_standby
                and os.environ.get("BITSLOTH_VLLM_STANDBY", "0") == "0"
            ):
                raise RuntimeError(
                    "Bitsloth: `bitsloth_vllm_standby` is True, but  environment variable `BITSLOTH_VLLM_STANDBY` is not set to 1!"
                )
        ...
        if DEVICE_TYPE == "cuda":
            ...
            raise ValueError(f"Bitsloth: Unsupported device type: {DEVICE_TYPE}")

        max_memory = round(gpu_stats.total_memory / 1024 / 1024 / 1024, 3)

        statistics = (
            f"==((====))==  Bitsloth {__version__}: Fast {model_patcher.__name__[4:-5]} patching. Transformers: {transformers_version}.{vllm_version}\n"
        ...
        if old_hf_transfer == "1":
            print(
                "Bitsloth: Fast downloading is enabled - ignore downloading bars which are red colored!"
            )
        ...
        if (rope_scaling is None) and (max_seq_length > model_max_seq_length):
            ...
            if fast_inference:
                raise NotImplementedError(
                    "Bitsloth: Fast inference does not yet work with RoPE Scaling."
                )

            logger.warning_once(
                f"Bitsloth: {model_name} can only handle sequence lengths of at most "
        ...
        # Log Bitsloth version for future fastpaths for inference
        ...
        if not is_classification and model.generate.__name__ != "bitsloth_fast_generate":
            model._old_generate = model.generate
            bitsloth_fast_generate.__doc__ = model._old_generate.__doc__
            model.generate = types.MethodType(bitsloth_fast_generate, model)
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        if os.environ.get("BITSLOTH_USE_NEW_MODEL", "0") == "1":
        ...
        if os.environ.get("BITSLOTH_ENABLE_FULL_FINETUNING", "0") == "1":
            print(
                "Bitsloth: Full finetuning is enabled, so .get_peft_model has no effect"
            )
            return model
        ...
        use_gradient_checkpointing = apply_bitsloth_gradient_checkpointing(
            use_gradient_checkpointing, max_seq, dtype
        )

        if type(r) is not int:
            raise TypeError(f"Bitsloth: Rank of {str(r)} must be an integer.")
        if r <= 0:
            raise TypeError(f"Bitsloth: Rank of {str(r)} must be larger than 0.")
        ...
        if isinstance(model, PeftModelForCausalLM) or isinstance(
            ...
            if check_all:
                # Simply pass through!
                logger.warning(
                    "Bitsloth: Already have LoRA adapters! We shall skip this step."
                )
                ...
                if "embed_tokens" in new_target_modules:
                    print(
                        "Bitsloth: Training embed_tokens in mixed precision to save VRAM"
                    )
                ...
                if "lm_head" in new_target_modules:
                    print("Bitsloth: Training lm_head in mixed precision to save VRAM")
                ...
                raise TypeError(
                    "Bitsloth: Your model already has LoRA adapters. Your new parameters are different."
                )
        ...
        if use_rslora:
            if not SUPPORTS_RSLORA:
                ...
                raise RuntimeError(
                    f"Bitsloth: Your PEFT version of {peft.__version__} does not support `use_rslora`.\n"
                    "Please install PEFT 0.7.2 or higher.\n"
                    "You can also install from source: `pip install git+https://github.com/huggingface/peft.git"
                )
        ...
        for module in target_modules:
            if module == "embed_tokens":
                # logger.warning_once(
                #     "Bitsloth: `embed_tokens` should be placed in `modules_to_save` and not `target_modules`. "\
                ...
                try:
                    ...
                    final_modules.append(module)
                    print(
                        "Bitsloth: You added custom modules, but Bitsloth hasn't optimized for this.\n"
        ...
        if hasattr(model, "_need_to_train_embeddings"):
            ...
            if not _lm_head_already_trained or not _embed_already_trained:
                print(
                    "Bitsloth: You added new tokens but did not specify if you wanted to "
        ...
        if modules_to_save is not None:
            for module in modules_to_save:
                if module == "lm_head":
                    ...
                    raise TypeError(
                        f"Bitsloth: Module = {module} is not allowed. Only 'lm_head' and 'embed_tokens' is allowed."
                    )
        ...
        if hasattr(model, "vllm_engine"):
            ...
            if modules_to_save is not None:
                raise NotImplementedError(
                    "Bitsloth: Currently fast inference does not work with training embeddings or lm_head."
                )

            if bias != "none":
                raise NotImplementedError(
                    "Bitsloth: Currently fast inference does not work with using biases for LoRA."
                )
        ...
        if use_gradient_checkpointing == "bitsloth":
            if train_embed_tokens:
                print("Bitsloth: Offloading input_embeddings to disk to save VRAM")
            ...
            if train_lm_head:
                print("Bitsloth: Offloading output_embeddings to disk to save VRAM")
        ...
        if qat_scheme is not None:
            print("Bitsloth: Applying QAT to mitigate quantization degradation")
        ...
        if ensure_weight_tying:
            try:
                    ...
                    def _retie_parameter(target_module, source_module):
                        ...
                        if hasattr(target_module, "weight"):
                            try:
                                ...
                                logger.warning_once(
                                    f"Bitsloth: Could not delete existing weight attr during retie on "
                ...
                logger.warning_once(
                    f"Bitsloth: Failed to ensure weight tying between embeddings and lm_head: {e}"
                )

        if train_embed_tokens:
            print("Bitsloth: Training embed_tokens in mixed precision to save VRAM")
    ...
    def patch_peft_model(
        ...
        if not isinstance(model, PeftModelForCausalLM) and not isinstance(
            ...
            raise TypeError(
                "Bitsloth: Your model needs to call `.get_peft_model` first!"
            )
        ...
        if model_type == "llama":
            ...
            raise NotImplementedError(f"Bitsloth: {model_type} is not yet implemented!")
        ...
        if Trainer._inner_training_loop.__name__ != "_fast_inner_training_loop":
            raise RuntimeError(
                "Bitsloth: Unsuccessfully patched Trainer! Please file a bug report!"
            )
            ...
            for idx, layer in enumerate(model.model.model.layers):
                if model_type != "falcon_h1":
                    ...
                    if (
                        ...
                        if hasattr(mlp_module, "_bitsloth_forward"):
                            # then we've patched the mlp to use TiledMLP
                            mlp_module._bitsloth_forward = types.MethodType(
                        ...
                        logger.warning_once(
                            "Not an error, but Bitsloth cannot patch MLP layers with our manual autograd engine since either LoRA adapters\n"
                ...
                if (
                    ...
                    if model_type == "qwen2":
                        ...
                        logger.warning_once(
                            "Not an error, but Bitsloth cannot patch Attention layers with our manual autograd engine since either LoRA adapters\n"
                            "are not enabled or a bias term (like in Qwen) is used."
                        )
                ...
                if (
                    ...
                    logger.warning_once(
                        "Not an error, but Bitsloth cannot patch O projection layer with our manual autograd engine since either LoRA adapters\n"
                        "are not enabled or a bias term (like in Qwen) is used."
                    )

        logger.warning_once(
            f"Bitsloth {__version__} patched {len(model.model.model.layers)} layers with "
    ...
    def for_inference(model):
        if not hasattr(model, "parameters"):
            raise TypeError(
                "Bitsloth: I think you're passing a tokenizer, not the model to for_inference!"
            )
    ...
    def for_training(model, use_gradient_checkpointing = True):
        if not hasattr(model, "parameters"):
            raise TypeError(
                "Bitsloth: I think you're passing a tokenizer, not the model to for_training!"
            )
