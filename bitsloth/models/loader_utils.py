...
def __get_model_name(
    ...
    if load_in_fp8 != False:
        if load_in_fp8 == True and (os.environ.get("BITSLOTH_HAS_FBGEMM", "0") == "1"):
        ...
        print(
            f"Bitsloth: Your transformers version of {transformers_version} does not support native "
        ...
        # logger.warning_once(
        #     f"Bitsloth: You passed in `{model_name}` which is a 4bit model, yet you set\n"\
        #     f"`load_in_4bit = False`. We shall load `{new_model_name}` instead."
        # )
        return new_model_name

    elif not load_in_4bit and lower_model_name in MAP_TO_BITSLOTH_16bit:
        new_model_name = MAP_TO_BITSLOTH_16bit[lower_model_name]
        return new_model_name
        ...
        # logger.warning_once(
        #     f"Bitsloth: You passed in `{model_name}` and `load_in_4bit = True`.\n"\
...
def _get_new_mapper():
    try:
        ...
        new_mapper = (
            ...
            .replace("MAP_TO_BITSLOTH_16bit", "NEW_MAP_TO_BITSLOTH_16bit")
        )
...
def _resolve_with_mappers(
    ...
    return __get_model_name(
        ...
        MAP_TO_BITSLOTH_16bit=map_to_bitsloth_16bit,
...
def get_model_name(
    ...
    if (
        ...
        # Try checking if a new Bitsloth version allows it!
        NEW_INT_TO_FLOAT_MAPPER, NEW_FLOAT_TO_INT_MAPPER, NEW_MAP_TO_BITSLOTH_16bit = (
            _get_new_mapper()
        )
        upgraded_model_name = _resolve_with_mappers(
            ...
            map_to_bitsloth_16bit=NEW_MAP_TO_BITSLOTH_16bit,
        )
...
def _offline_quantize_to_fp8(model_name: str, fp8_mode: str) -> str:
    ...
    print(
        f"Bitsloth: Quantizing '{model_name}' to fp8, using model_name='{new_model_name}' instead"
    )
...
def _get_fp8_mode_and_check_settings(
    ...
    if fp8_mode not in ["row", "block"]:
        raise ValueError(
            f"Bitsloth: `load_in_fp8` can only be 'row' or 'block', got '{fp8_mode}'"
        )
    if full_finetuning:
        raise ValueError(
            "Bitsloth: `load_in_fp8` is not compatible with full finetuning"
        )
    if load_in_4bit or load_in_8bit or load_in_16bit:
        raise ValueError(
            "Bitsloth: `load_in_fp8` is not compatible with `load_in_4bit`, `load_in_8bit` or `load_in_16bit`",
        )
    ...
    if not (
        ...
        raise ValueError(
            "Bitsloth: On the fly `load_in_fp8` requires H100 GPUs or after. Try `bitsloth/Qwen3-8B` instead."
        )

    # Check if torch >= 2.9.0
    if Version(torch.__version__) < Version("2.9.0"):
        raise ValueError(
            "Bitsloth: On the fly `load_in_fp8` requires torch 2.9.0+. Try `bitsloth/Qwen3-8B` instead."
        )
    ...
    if importlib.util.find_spec("torchao") is None:
        raise ValueError(
            "Bitsloth: Please install torchao for on the fly float8 to work! Try `bitsloth/Qwen3-8B` instead."
        )
    import torchao

    error_message = (
        "Bitsloth: `load_in_fp8` requires torchao 0.15.0+ (or nightly).\n"
