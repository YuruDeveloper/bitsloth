...
__all__ = [
    ...
    "apply_bitsloth_gradient_checkpointing",
...
from bitsloth_zoo.utils import Version, get_quant_type
...
from ..import_fixes import BITSLOTH_ENABLE_LOGGING
...
from bitsloth_zoo.patching_utils import (
    patch_compiling_bitsandbytes,
    patch_layernorm,
    patch_torch_compile,
    patch_model_and_tokenizer,
...
from bitsloth_zoo.compiler import (
    get_transformers_model_type,
    bitsloth_compile_transformers as _bitsloth_compile_transformers,
)
...
from bitsloth_zoo.temporary_patches import (
    TEMPORARY_PATCHES,
)


def apply_bitsloth_gradient_checkpointing(
    use_gradient_checkpointing, max_seq_length, dtype
):
    """
    Apply gradient checkpointing with smart heuristics.

    For seq < 512, the overhead of gradient offloading in gc="bitsloth" mode
    is not worth it. Benchmarks show standard gc is faster for small sequences.

    Args:
        use_gradient_checkpointing: "bitsloth", True, False, or None
        max_seq_length: The maximum sequence length
        dtype: The model dtype for patching

    Returns:
        The effective use_gradient_checkpointing value (may change from "bitsloth" to True)
    """
    if use_gradient_checkpointing == "bitsloth":
        ...
        if max_seq_length < 512:
            ...
            patch_bitsloth_smart_gradient_checkpointing(dtype=dtype)
            return "bitsloth"
    elif use_gradient_checkpointing in (True, False):
        # User explicitly set True or False - unpatch any previous "bitsloth" patching
        unpatch_bitsloth_smart_gradient_checkpointing()
...
class ReplaceWarningMessage:
    """
    Intercepts warnings.warn calls and replaces matching messages with Bitsloth branded ones.
...
class _RaiseUninitialized(logging.Handler):
    ...
    def emit(self, record):
        ...
        if (
            ...
            and (os.environ.get("BITSLOTH_WARN_UNINITIALIZED", "1") == "1")
        ):
            raise Exception(
                f"Bitsloth: Critical error since some weights are not initialized.\n"
                f"Please try updating Bitsloth, transformers and timm via:\n"
                f"`pip install --upgrade --force-reinstall --no-cache-dir --no-deps bitsloth bitsloth_zoo transformers timm`\n"
                f"{str(record)}"
            )
...
# Replace PEFT target_parameters warning with Bitsloth branded message for MoE models
ReplaceWarningMessage.add_rule(
    match_text="target_parameters",
    replacement=(
        "Bitsloth: PEFT set target_parameters but found no matching parameters.\n"
        "This is expected for MoE models - Bitsloth handles MoE expert LoRA targeting separately."
    ),
...
#     source = source[:where] + \
#         f"if len(self) == 0:\n{spaces}{spaces}"\
#         "    raise RuntimeError('Bitsloth: You must call `FastLanguageModel.for_inference(model)` before doing inference for Bitsloth models.')\n" + \
...
if is_openai_available():
    try:
        ...
        print("Bitsloth: OpenAI failed to import - ignoring for now.")
        ...
        if _is_package_available("flash_attn"):
            ...
            try:
                ...
                if not HAS_FLASH_ATTENTION_SOFTCAPPING:
                    print(
                        "Bitsloth: If you want to finetune Gemma 2, upgrade flash-attn to version 2.6.3 or higher!\n"
                ...
                print(
                    "Bitsloth: Your Flash Attention 2 installation seems to be broken. "
                ...
                transformers.utils.is_flash_attn_2_available = lambda *args, **kwargs: (
                    False
                )
...
try:
    ...
    if DEVICE_TYPE == "cuda":
        ...
        if (f"{major_version}.{minor_version}" in ("10.0", "11.0", "12.0")) and (
            ...
            raise NotImplementedError(
                f"Bitsloth: Xformers {xformers_version} has a broken FA3 dispatch on "
    ...
    if False:  # Version(xformers_version) >= Version("0.0.27"):
        raise ImportError(
            "Bitsloth: If you are in Colab, we updated the top cell install instructions - please change it to below "
    ...
    if Version(torch_version) < Version("2.2.0") and Version(
        ...
        raise ImportError(
            f"Bitsloth: You have torch = {torch_version} but xformers = {xformers_version}.\n"
    ...
    try:
        ...
        raise ImportError(
            "Bitsloth: Xformers was not installed correctly.\n"
...
# Bitsloth now supports all TRL versions!
...
BITSLOTH_COMPILE_IGNORE_ERRORS = (
    os.environ.get("BITSLOTH_COMPILE_IGNORE_ERRORS", "1") == "1"
)
...
patch_torch_compile(
    ...
    O3=BITSLOTH_COMPILE_MAXIMUM,
    ignore_errors=BITSLOTH_COMPILE_IGNORE_ERRORS,
)
...
torch_compile_options = {
    ...
    "max_autotune": True,
    "shape_padding": True,
    "trace.enabled": BITSLOTH_COMPILE_DEBUG,
...
def torch_compile_kwargs(*args, **kwargs):
    print("Bitsloth: Enabled auto compiling")
    return {
        "dynamic": True,
        "fullgraph": False,
...
def patch_regional_compilation():
    ...
    if torch.nn.ModuleList.__name__ == "BitslothModuleList":
        return
    ...
    def BitslothModuleList(*args, **kwargs):
    ...
    BitslothModuleList.__doc__ = old_module_list.__doc__

    torch.nn.ModuleList = BitslothModuleList
    return
...
if Version(peft_version) < Version("0.12.0"):
    ...
    try:
        ...
        logger.warning_once(
            "Bitsloth unsuccessfully patched LoraLayer.update_layer. Please file a bug report.\n"
...
USE_MODELSCOPE = os.environ.get("BITSLOTH_USE_MODELSCOPE", "0") == "1"
if USE_MODELSCOPE:
...
def _get_statistics(statistics=None, force_download=True):
    ...
    if statistics is not None:
        ...
        from bitsloth_zoo.rl_environments import execute_with_time_limit
        ...
        if has_internet():
            ...
            try:
                ...
                raise TimeoutError(
                    "Bitsloth: HuggingFace seems to be down after trying for 120 seconds :(\n"
...
def get_statistics(local_files_only=False):
    ...
    # We simply download a README.md file from HF - all data is made public.
    # This is simply so we can check if some envs are broken or not.
    # You can disable this by setting BITSLOTH_DISABLE_STATISTICS
    import os

    if (
        "BITSLOTH_DISABLE_STATISTICS" in os.environ
        or os.environ.get("BITSLOTH_USE_MODELSCOPE", "0") == "1"
    ):
        return
    if local_files_only:
        return
...
if DEVICE_COUNT == 1 and int(os.environ.get("WORLD_SIZE", "1")) <= 1:
    ...
    accelerate.accelerator.Accelerator.distributed_type = lambda *args, **kwargs: (
        DistributedType.NO
    )
...
def offload_to_disk(
    W, model, name, temporary_location: str = "_bitsloth_temporary_saved_buffers"
):
...
def offload_input_embeddings(
    model, temporary_location: str = "_bitsloth_temporary_saved_buffers"
):
...
def _bitsloth_pre_compute_loss(self, model, inputs, *args, **kwargs):
    ...
    if (
        ...
        logger.warning_once(
            f"Bitsloth: Not an error, but {name} does not accept `num_items_in_batch`.\n"
            "Using gradient accumulation will be very slightly less accurate.\n"
            "Read more on gradient accumulation issues here: https://bitsloth.ai/blog/gradient"
        )
...
def patch_gradient_accumulation_fix(Trainer):
    ...
    # Fixes Output 0 of BitslothFusedLossBackward is a view and is being modified inplace.
    import inspect

    if hasattr(Trainer, "get_batch_samples"):
        if Trainer.get_batch_samples.__name__ == "_bitsloth_get_batch_samples":
            return
        if (
            ...
            raise NotImplementedError(
                "Bitsloth: Please make a Github issue immediately!!"
            )
        else:
            if Trainer.get_batch_samples.__name__ != "_bitsloth_get_batch_samples":
                Trainer.get_batch_samples = _bitsloth_get_batch_samples

            # Also fix passing in num_items_in_batch
            if not hasattr(Trainer, "_old_compute_loss"):
                # Fix transformers 4.57.0 causing `Output 0 of BitslothFusedLossBackward is a view and is being modified inplace.`
                ...
                Trainer.compute_loss = _bitsloth_pre_compute_loss
    else:
        logger.warning_once(
            "Bitsloth: We fixed a gradient accumulation bug, "
            "but it seems like you don't have the latest transformers version!\n"
            "Please update transformers, TRL and bitsloth via:\n"
            "`pip install --upgrade --no-cache-dir --no-deps bitsloth transformers git+https://github.com/huggingface/trl.git`"
        )

    # Also fix up loss scaling ie negate loss *= self.args.gradient_accumulation_steps
    if not (
        Trainer.training_step.__name__ == "_bitsloth_training_step"
        ...
        function = function.replace(
            "def training_step", "def _bitsloth_training_step", 1
        )
        ...
        exec(function, globals())
        Trainer.training_step = _bitsloth_training_step
    ...
    if Trainer.__init__.__name__ != "_bitsloth___init__":
        try:
        ...
        if init_function is not None:
            ...
            init_function = init_function.replace(
                "def __init__", "def _bitsloth___init__", 1
            )
            ...
            Trainer.__init__ = _bitsloth___init__


def patch_tokenizer(model, tokenizer):
    model, tokenizer = _patch_tokenizer(model, tokenizer)
    if model is not None:
        model.config.update({"bitsloth_version": __version__})
...
def bitsloth_compile_transformers(
    ...
    disable=False,
    return_logits=False,
    bitsloth_force_compile=False,
):
    if Version(torch_version) < Version("2.4.0"):
        print(
            "="
            * 30
            + "Bitsloth: Unfortunately Bitsloth vision and other newer optimized models need Torch 2.4 or later.\n"
        ...
        return
    if trust_remote_code and bitsloth_force_compile == False:
        print(
            "Bitsloth: We can't trace models if `trust_remote_code = True`, "
    ...
    for model_type in model_types:
        _bitsloth_compile_transformers(
            model_type,
...
def validate_loftq_config(loftq_config, lora_dropout, bias, init_lora_weights, model):
    ...
    if lora_dropout != 0:
        logger.warning_once(
            f"Bitsloth: Dropout = 0 is supported for fast patching. You are using dropout = {lora_dropout}.\n"
            f"Bitsloth will patch all other layers, except LoRA matrices, causing a performance hit."
        )

    if bias != "none":
        logger.warning_once(
            f"Bitsloth: bias = `none` is supported for fast patching. You are using bias = {bias}.\n"
            f"Bitsloth will patch all other layers, except LoRA matrices, causing a performance hit."
        )
    ...
    if not (
        ...
        raise ValueError(
            'Bitsloth: `init_lora_weights` must be either [True, False, "gaussian", "loftq", "corda"].'
        )
    ...
    if init_lora_weights == "loftq":
        if not SUPPORTS_LOFTQ:
            ...
            raise RuntimeError(
                f"Bitsloth: Your PEFT version of {peft.__version__} does not support LoftQ init.\n"
        ...
        if loftq_config == {}:
            ...
            logger.warning_once(
                "Bitsloth: init_lora_weights = `loftq` is set, but `loftq_config` is None.\n"
        ...
        if hasattr(model.config, "quantization_config"):
            raise ValueError(
                "Bitsloth: You are using `loftq` init, yet `load_in_4bit = True` was set.\n"
...
def fast_inference_setup(model_name, model_config):
    ...
    if not is_vLLM_available():
        logger.warning_once(
            "Bitsloth: vLLM is not installed! Will use Bitsloth inference!"
        )
    ...
    patch_vllm()
    if model_name.endswith("bitsloth-bnb-4bit"):
        if not vllm_dynamic_quant_supported(model_name, model_config):
            # Instead use -bnb-4bit variant
            logger.warning_once(
                f"Bitsloth: Switching from Bitsloth dynamic quant to normal quant since\n"
                f"we do not yet support fast inference for {model_name}"
            )
            model_name = model_name[: -len("bitsloth-bnb-4bit")] + "bnb-4bit"
...
def patch_peft_fast_inference(model):
    ...
    if vllm_engine is not None:
        ...
        from bitsloth_zoo.vllm_utils import save_lora, load_lora
...
def error_out_no_vllm(*args, **kwargs):
    raise NotImplementedError(
        "Bitsloth: vLLM is not yet supported for fast inference for this model! Please use `.generate` instead"
    )
...
try:
    ...
    try:
        ...
        print("Bitsloth: TorchAO changed `torchao.quantization.Int4WeightOnlyConfig`")
...
def _prepare_model_for_qat(
    ...
    if qat_scheme == "int8-int4":
        ...
        if is_gemma3:
            print(
                "Bitsloth: Gemma3 has a large vocabulary causing int8 embedding issues. "
    ...
    if not isinstance(qat_scheme, TorchAOConfig):
        ...
        if qat_scheme == "fp8-int4":
            ...
            filter_fn = lambda m, _: (
                isinstance(m, torch.nn.Linear) and m.in_features >= group_size
            )
...
def verify_fp8_support_if_applicable(model_config):
    ...
    if quant_method in ["fbgemm_fp8", "fp8"] and DEVICE_TYPE != "cuda":
        raise ValueError(
            f"Bitsloth: FP8 quantization is only supported on CUDA GPUs. You are using {DEVICE_TYPE}."
        )
    ...
    if DEVICE_TYPE == "cuda":
        ...
        if quant_method == "fbgemm_fp8" and major_version < 9:
            ...
            raise ValueError(
                f"Bitsloth: FBGEMM FP8 quantization is only supported on H100 and higher GPUs. L4 is not supported. You are using {torch.cuda.get_device_name()}. Refer to https://developer.nvidia.com/cuda-gpus for more details."
            )
        if quant_method == "fp8" and major_version * 10 + minor_version < 89:
            ...
            raise ValueError(
                f"Bitsloth: FP8 quantization is only supported on L4 and higher GPUs with compute capability 8.9 or higher. You are using {torch.cuda.get_device_name()}. Refer to https://developer.nvidia.com/cuda-gpus for more details."
            )
...
def get_moe_target_parameters(model, target_modules=None) -> Optional[List[str]]:
    ...
    if moe_params:
        print(
            f"Bitsloth: Detected MoE model with {num_experts = } and {target_modules = }. Enabling LoRA on MoE parameters: {moe_params}"
        )
        return moe_params
...
def make_fast_generate_wrapper(original_generate):
    ...
    def _fast_generate_wrapper(*args, **kwargs):
        ...
        if "sampling_params" in kwargs:
            raise ValueError(
                "Bitsloth: `sampling_params` is only supported when `fast_inference=True` (vLLM). "
        ...
        if "lora_request" in kwargs:
            raise ValueError(
                "Bitsloth: `lora_request` is only supported when `fast_inference=True` (vLLM). "
        ...
        if len(args) > 0:
            ...
            if is_string_input:
                raise ValueError(
                    "Bitsloth: Passing text strings to `fast_generate` is only supported "
...
# Dynamic quant checkpoints (eg gemma-3-4b-it-bitsloth-bnb-4bit) encode skip paths as
