...
try:
    import vllm.sampling_params as _bitsloth_vllm_sp

    if not hasattr(_bitsloth_vllm_sp, "GuidedDecodingParams"):

        class GuidedDecodingParams:
            def __init__(self, **kwargs):
                self.kwargs = kwargs

        _bitsloth_vllm_sp.GuidedDecodingParams = GuidedDecodingParams
except Exception:
    pass
...
def _patch_resume_from_checkpoint_memory(trainer_class):
    ...
    if original_train is None:
        return
    if getattr(original_train, "_bitsloth_resume_guard", False):
        return

    def _bitsloth_train_with_resume_guard(self, *args, **kwargs):
    ...
    _bitsloth_train_with_resume_guard._bitsloth_resume_guard = True
    trainer_class.train = _bitsloth_train_with_resume_guard
...
def PatchRL(FastLanguageModel):
    ...
    @contextmanager
    def bitsloth_unwrap_model_for_generation(model, *args, **kwargs):
        with unwrap_model_for_generation(model, *args, **kwargs) as unwrapped_model:
            # Put the model in inference mode.
            FastLanguageModel.for_inference(model)

            # We must use .clone for Bitsloth since we force inference_mode
    ...
    def bitsloth_prediction_step(
        ...
        os.environ["BITSLOTH_RETURN_LOGITS"] = "0"
    ...
    for trainer in trainers:
        ...
        if hasattr(current_trainer, unwrap):
            try:
                setattr(current_trainer, unwrap, bitsloth_unwrap_model_for_generation)
            except:
                continue
    Trainer.prediction_step = bitsloth_prediction_step
...
def prepare_for_training_mode(f):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        self._bitsloth_training_completed = True
        return output
    return wrapper
pass
...
@dataclass
class Bitsloth{RLConfig_name}({RLConfig_name}):
    """
    {__RLConfig_doc__}
    """
    ...
    bitsloth_num_chunks : Optional[int] = field(
        default = -1,
        metadata = {{'help': 'Chunk size to reduce memory usage. -1 is most efficient.'}},
    )
    bitsloth_logit_chunk_multiplier : Optional[int] = field(
            default = None,
            metadata = {{'help': 'Multiplier for chunked logit computations.'}},
        )
    bitsloth_grpo_mini_batch : Optional[int] = field(
        default = None,
    ...
    def __init__({RLConfig_arguments},
        ...
        bitsloth_num_chunks = -1,
        bitsloth_logit_chunk_multiplier = None,
        bitsloth_grpo_mini_batch = None,
        {max_seq_length_call}
        **kwargs,
    ):
{RLConfig_extra_args}
        ...
        self.bitsloth_num_chunks = bitsloth_num_chunks
        if bitsloth_grpo_mini_batch is not None:
            if self.generation_batch_size >= bitsloth_grpo_mini_batch:
                self.bitsloth_grpo_mini_batch = bitsloth_grpo_mini_batch
            else:
                raise ValueError(
                    f"Bitsloth GRPO mini batch size needs to be less than or equal to the effective generation batch size, "
                    f"which is self.per_device_train_batch_size * gradient_accumulation_steps."
                )
        self.bitsloth_logit_chunk_multiplier = bitsloth_logit_chunk_multiplier
        {max_seq_length_post}
{RLConfig_post}
pass
...
class Bitsloth{RLTrainer_name}(_Bitsloth{RLTrainer_name}):
    """
    {__RLTrainer_doc__}
    """
    def __init__({RLTrainer_arguments},
        **kwargs
        ...
        if args is None: args = Bitsloth{RLConfig_name}()
...
def _wrap_grpo_generate_and_score(trainer_cls):
    ...
    if getattr(original, "_bitsloth_restore_training_wrapped", False):
        return
    ...
    wrapped._bitsloth_restore_training_wrapped = True
...
def _patch_trl_rl_trainers(trainer_file="grpo_trainer"):
    ...
    try:
        ...
        logger.info(f"Bitsloth: Could not import trl.trainer.{trainer_file}: {error}")
        return
    ...
    if len(name) != 1:
        logger.info(
            f"Bitsloth: Could not find Trainer class in trl.trainer.{trainer_file}. Found: {name}"
        )
        return
    if len(config) != 1:
    ...
    if len(config) != 1:
        logger.info(
            f"Bitsloth: Could not find Config class in trl.trainer.{trainer_file}. Found: {config}"
        )
        return
    ...
    try:
        ...
        logger.info(
            f"Bitsloth: Could not load {RLTrainer_name} from trl.trainer.{trainer_file}: {e}"
        )
        return
    ...
    try:
        ...
        try:
            ...
            if not _config_loaded:
                logger.info(f"Bitsloth: Could not load {RLConfig_name}")
                return

    # Check name
    if RLTrainer.__name__.startswith("Bitsloth"):
        print(f"Bitsloth: {RLTrainer.__name__} is already patched.")
        return
    if RLConfig.__name__.startswith("Bitsloth"):
        print(f"Bitsloth: {RLConfig.__name__} is already patched.")
        return
    ...
    if "args" in call_args and "model" in call_args:
        mixed_precision = (
            ...
            "force_float32 = False\n"
            "full_finetuning = os.environ.get('BITSLOTH_ENABLE_FULL_FINETUNING', '0') == '1'\n"
            "if not full_finetuning and (os.environ.get('BITSLOTH_FORCE_FLOAT32', '0') == '1'):\n"
            "    print('Bitsloth: Switching to float32 training since model cannot work with float16')\n"
            "    force_float32 = True\n"
            ...
            "if not force_float32 and (not float16 and use_fp16): raise TypeError('Bitsloth: Model is in bfloat16 precision but you want to use float16 precision. Set fp16 to `False` and bf16 to `True`')\n"
            "if force_float32:\n"
    ...
    if "args" in call_args:
        ...
        eval_changes = (
            ...
            "elif os.environ.get('BITSLOTH_MIXED_PRECISION', 'float32') == 'bfloat16':\n"
    ...
    if "model" in call_args:
        length_check = (
            ...
            "            print('Bitsloth: You set `max_seq_length` as ' + str(args_max_seq_length) + ' but '\n"
        ...
        if trainer_file == "sft_trainer":
            max_length_check = (
                ...
                "        else:\n"
                "            print('Bitsloth: We did not find `max_seq_length` or `max_length` in the model or args. We will set it to 1024.')\n"
    ...
    if "learning_rate" in call_args:
        learning_rate_check = (
            "if learning_rate < 1e-7: print(f'Bitsloth: Your learning rate of `{learning_rate}` is too small and less than 1e-7! "
            "Consider increasing it, otherwise gradient updates will be close to 0!')\n"
            "if learning_rate > 1: print(f'Bitsloth: Your learning rate of `{learning_rate}` is way too larger > 1! "
    ...
    if "output_dir" in call_args:
        ...
        saving_check = (
            ...
            "    output_dir = 'bitsloth_training_checkpoints'\n"
            "    save_strategy = 'no'\n"
        )
    ...
    if "loss_type" in call_args and "scale_rewards" in call_args:
        ...
        check_dr_grpo = (
            ...
            "        print('Bitsloth: The Dr GRPO paper recommends setting `scale_rewards` to False! Will override. Set it to `None` to force False.')\n"
            "        scale_rewards = False\n"
            "elif loss_type.lower() == 'dapo':\n"
            "    if mask_truncated_completions != True:\n"
            "        print('Bitsloth: The DAPO paper recommends `mask_truncated_completions = True` - we will set it.')\n"
            "    if epsilon_high != 0.28:\n"
            "        print('Bitsloth: The DAPO paper recommends `epsilon_high = 0.28` - we will set it.')\n"
            "    if beta != 0.0:\n"
            "        print(f'[WARNING] Bitsloth: The DAPO paper recommends setting `beta = 0.0` to remove the KL term - You have set it to {beta}.')\n"
    ...
    if (
        ...
        check_num_generations = (
            ...
            "        print('Bitsloth: We now expect `per_device_train_batch_size` * `gradient_accumulation_steps` * `world_size` to be a multiple of `num_generations`.\\n"
        ...
        if "steps_per_generation" not in call_args:
            print(f"Bitsloth: Could not find `steps_per_generation` in {trainer_file}")
        if "generation_batch_size" not in call_args:
            print(f"Bitsloth: Could not find `generation_batch_size` in {trainer_file}")

        check_num_generations = (
            "if (per_device_train_batch_size // num_generations) * num_generations != per_device_train_batch_size:\n"
            "    print('Bitsloth: We now expect `per_device_train_batch_size` to be a multiple of `num_generations`.\\n"
            "We will change the batch size of ' + str(per_device_train_batch_size) + ' to the `num_generations` of ' + str(num_generations))\n"
            "    per_device_train_batch_size = num_generations\n"
            "\n"
        )
    ...
    if "temperature" in call_args:
        check_temperature = (
            "if temperature <= 0:\n"
            "    raise ValueError('Bitsloth: Please set a positive non-zero temperature since your results will be wrong.')\n"
            "elif temperature >= 10:\n"
            "    raise ValueError('Bitsloth: Please set a positive non-zero temperature less than 10, since sampling will be quite erratic.')\n"
            "\n"
        )
    ...
    # Bitsloth gradient checkpointing requires use_reentrant=True, so we remove
    # the setting after super().__init__() when it gets auto-applied.
    RLConfig_post = ""
    if trl_version >= Version("0.27.0"):
        RLConfig_post = (
            "        # Bitsloth: Remove use_reentrant=False forced by TRL 0.27.0+\n"
    ...
    if RLTrainer_extras is None:
        RLTrainer_extras = f"_Bitsloth{RLTrainer_name} = {RLTrainer_name}"
    ...
    # and breaks GradScaler when training with fp16=True. Bitsloth already
    ...
    if RLTrainer_name == "SFTTrainer":
        ...
        _vlm_check_patched = (
            '        self._is_vision_dataset = "image" in dataset_sample or "images" in dataset_sample\n'
            "        # Bitsloth: override _is_vlm for VLM models that pass a bare tokenizer\n"
    ...
    # Patch Trainer
    exec(
        f"trl.{RLTrainer_name} = created_module.Bitsloth{RLTrainer_name}",
        locals(),
        globals(),
    )
    exec(
        f"trl.trainer.{RLTrainer_name} = created_module.Bitsloth{RLTrainer_name}",
        locals(),
        globals(),
    )
    exec(
        f"trl.trainer.{trainer_file}.{RLTrainer_name} = created_module.Bitsloth{RLTrainer_name}",
        locals(),
        globals(),
    )
    ...
    # Patch Config
    exec(
        f"trl.{RLConfig_name} = created_module.Bitsloth{RLConfig_name}",
        locals(),
        globals(),
    )
    exec(
        f"trl.trainer.{RLConfig_name} = created_module.Bitsloth{RLConfig_name}",
        locals(),
        globals(),
    )
    exec(
        f"trl.trainer.{trainer_file}.{RLConfig_name} = created_module.Bitsloth{RLConfig_name}",
        locals(),
        globals(),
    )
    ...
    if trainer_file == "grpo_trainer":
        try:
            _wrap_grpo_generate_and_score(
                getattr(created_module, f"Bitsloth{RLTrainer_name}")
            )
        except Exception as e:
            logger.info(
                f"Bitsloth: Could not wrap _generate_and_score_completions for {RLTrainer_name}: {e}"
            )
...
def patch_functions(RLTrainer, trainer_file, RLTrainer_name, all_imports, imports):
    ...
    # Bitsloth: We comment out the "ref" adapter creation because:
    ...
    def comment_out_block(match):
        ...
        commented_lines.append(
            f"{indent}# Bitsloth: Commented out - use base model as reference, not SFT/LoRA model"
        )
        ...
        if len(replacer) != 0:
            ...
            if "grpo" in trainer_file and trl_version >= Version("0.18.0"):
                ...
                if trl_version >= Version("0.23.0"):
                    ...
                    vllm_setter += (
                        " " * 12
                        + "if os.environ.get('BITSLOTH_VLLM_STANDBY', '0') == '1':\n"
                        + " " * 16
    ...
    for function in functions:
        ...
        # Prefer using bitsloth's sampling params and fallback to trl's if not found
    ...
    RLTrainer_source = RLTrainer_source.replace(
        f"class {RLTrainer_name}", f"class _Bitsloth{RLTrainer_name}", 1
    )
...
def patch_trl_rl_trainers():
    ...
    for trainer in all_trainers:
        try:
            ...
            logger.warning_once(f"Bitsloth: Could not patch trl.trainer.{trainer}: {e}")
    return
...
def patch_trl_vllm_generation():
    ...
    for function in RL_ADDITIONAL_FUNCTIONS["vllm_generation"]:
        logger.info(
            f"Bitsloth: Patching trl VLLMGeneration with function: {function.__name__}"
        )
        function()
    return
