...
def _fix_rope_inv_freq(model):
    ...
    hasattr(module, "original_inv_freq"). Bitsloth's LlamaRotaryEmbedding and
    ...
    for name, module in model.named_modules():
        # Bitsloth's LlamaRotaryEmbedding and subclasses (Extended, LinearScaling,
        ...
        **kwargs,
        ...
        if fast_inference:
            if importlib.util.find_spec("vllm") is None:
                raise ImportError(
                    "Bitsloth: Please install vLLM before enabling `fast_inference`!\n"
            ...
            if DEVICE_TYPE_TORCH == "cuda":
                for i in range(DEVICE_COUNT):
                    ...
                    if "NVIDIA GB10" in str(torch.cuda.get_device_name(i)).upper():
                        print(
                            "Bitsloth: DGX Spark detected - `fast_inference=True` is currently broken as of January 2026.\n"
                            "Defaulting to native Bitsloth inference."
                        )
        ...
        if not ALLOW_BITSANDBYTES and not use_exact_model_name:
            if load_in_4bit or load_in_8bit or model_name.lower().endswith("-bnb-4bit"):
                print(
                    "Bitsloth: AMD currently is not stable with 4bit bitsandbytes. Disabling for now."
                )
            load_in_4bit = False
        ...
        if not ALLOW_PREQUANTIZED_MODELS and model_name.lower().endswith(
            ("-bitsloth-bnb-4bit", "-bnb-4bit")
        ):
            model_name = model_name.lower().removesuffix("-bitsloth-bnb-4bit")
        ...
        try:
            ...
            if "architecture" in autoconfig_error:
                if "qwen3_5" in autoconfig_error:
                    raise ImportError(
                        f"Bitsloth: Your transformers version of {transformers_version} does not support Qwen3.5.\n"
        ...
        if both_exist:
            raise RuntimeError(
                "Bitsloth: Your repo has a LoRA adapter and a base model.\n"
        ...
        if not is_model and not is_peft:
            ...
            if "rope_scaling" in error.lower() and not SUPPORTS_LLAMA31:
                raise ImportError(
                    f"Bitsloth: Your transformers version of {transformers_version} does not support new RoPE scaling methods.\n"
            ...
            combined_error = (
                "Bitsloth: Failed to load model. Both AutoConfig and PeftConfig loading failed.\n\n"
        ...
        if model_type == "llama":
            ...
            if scaling_type == "llama3" and not SUPPORTS_LLAMA31:
                raise ImportError(
                    f"Bitsloth: Your transformers version of {transformers_version} does not support Llama 3.1.\n"
            ...
            if not SUPPORTS_GEMMA:
                raise ImportError(
                    f"Bitsloth: Your transformers version of {transformers_version} does not support Gemma.\n"
            ...
            if not SUPPORTS_GEMMA2:
                raise ImportError(
                    f"Bitsloth: Your transformers version of {transformers_version} does not support Gemma2.\n"
            ...
            if is_bfloat16_supported() and not HAS_FLASH_ATTENTION:
                print(
                    "Bitsloth: If you want to finetune Gemma 2, install flash-attn to make it faster!\n"
            ...
            if not SUPPORTS_QWEN3 or not SUPPORTS_QWEN3_MOE:
                raise ImportError(
                    f"Bitsloth: Your transformers version of {transformers_version} does not support Qwen3.\n"
        ...
        #         raise ImportError(
        #             f"Bitsloth: Your transformers version of {transformers_version} does not support FalconH1.\n"\
        #             f"The minimum required version is 4.50.3.\n"\
        #             f'Try `pip install --upgrade "transformers>=4.50.3"`\n'\
        #             f"to obtain the latest transformers build, then restart this session."\
        #         )
        ...
        # In case the model supports tagging, add the bitsloth tag.
        ...
        # Patch Tiled MLP
        # to turn on set BITSLOTH_TILED_MLP to "arctic", "target", or "target:{GB}""
        patch_tiled_mlp_choice = os.environ.get(
            "BITSLOTH_TILED_MLP", "arctic" if bitsloth_tiled_mlp else "0"
        )
        if patch_tiled_mlp_choice != "0" or bitsloth_tiled_mlp:
...
class FastModel(FastBaseModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        string = os.environ.get("BITSLOTH_MODEL_NAME", "") + model_types_all
        if load_in_4bit:
        ...
        if load_in_fp8:
            string += "load_in_fp8"
        os.environ["BITSLOTH_MODEL_NAME"] = string
        ...
        if "pixtral" in model_types_all and transformers_version < Version("4.49.0"):
            raise RuntimeError(
                "Bitsloth: Pixtral only works on transformers >= 4.49.0." + LATEST
            )
        # Qwen 2.5
        elif "qwen2_5" in model_types_all and transformers_version < Version("4.49.0"):
            raise RuntimeError(
                "Bitsloth: Qwen 2.5 only works on transformers >= 4.49.0." + LATEST
            )
        # Gemma 3N must be before Gemma 3
        elif "gemma3n" in model_types_all:
            if transformers_version < Version("4.53.0"):
                raise RuntimeError(
                    "Bitsloth: Gemma 3N only works on transformers >= 4.53.0" + LATEST
                )
            os.environ["BITSLOTH_DISABLE_STATIC_GENERATION"] = "1"
            os.environ["BITSLOTH_FORCE_CUSTOM_DTYPE"] = (
            ...
            os.environ["BITSLOTH_HIGH_PRECISION_LAYERNORM"] = "1"
        # Gemma 3
        elif "gemma3" in model_types_all:
            if transformers_version < Version("4.50.0.dev0"):
                raise RuntimeError(
                    "Bitsloth: Gemma 3 only works on transformers >= 4.50.0." + NIGHTLY
                )
            # Set norms to float32 since anyways they get upcasted to float32
            # common in both gemma-3 and gemma-3n
            os.environ["BITSLOTH_HIGH_PRECISION_LAYERNORM"] = "1"
            ...
            if is_rdna():
                os.environ["BITSLOTH_COMPILE_DISABLE"] = "partial"
        # Cohere
        elif "cohere2" in model_types_all and transformers_version < Version(
            "4.50.0.dev0"
        ):
            raise RuntimeError(
                "Bitsloth: Cohere's Command model only works on transformers >= 4.50.0."
                + NIGHTLY
            )
        # Sesame
        elif "csm" in model_types_all:
            os.environ["BITSLOTH_COMPILE_DISABLE"] = "partial"  # Inference is too slow
            os.environ["BITSLOTH_DISABLE_STATIC_GENERATION"] = "1"  # Sesame fails
            os.environ["BITSLOTH_FORCE_CUSTOM_DTYPE"] = (
        ...
        # Olmo 2
            ...
            raise RuntimeError(
                "Bitsloth: OLMo-2 only works on transformers >= 4.50.0." + NIGHTLY
            )
            ...
            for check_model_name in DISABLE_COMPILE_MODEL_NAMES:
                if check_model_name in lowered_model_name:
                    ...
                    if transformers_version < Version("4.50.0.dev0"):
                        raise RuntimeError(
                            f"Bitsloth: {check_model_name} only works on transformers >= 4.50.0."
                            + NIGHTLY
                        )
                    break
        ...
        os.environ["BITSLOTH_FORCE_FLOAT32"] = "0"
        ...
        for disable_name in FORCE_FLOAT32:
            ...
            if (
                ...
                os.environ["BITSLOTH_FORCE_FLOAT32"] = "1"
        ...
        with redirector:
            ...
            model_types, supports_sdpa = bitsloth_compile_transformers(
                dtype = dtype,
                model_name = model_name,
                ...
                trust_remote_code = trust_remote_code,
                bitsloth_force_compile = bitsloth_force_compile,
            )
        # Fix SDPA issues
