...
from ..save import bitsloth_save_pretrained_torchao, bitsloth_save_pretrained_gguf
import contextlib
import shutil
...
def _save_pretrained_torchao(
    ...
    def patch_bitsloth_save():
        original_causal = transformers.AutoModelForCausalLM
        original_rmtree = shutil.rmtree
        # bitsloth_save_pretrained_torchao expects AutoModelForCausalLM
        transformers.AutoModelForCausalLM = transformers.AutoModel
        # prevent bitsloth from deleting the unquantized model directory
    ...
    with patch_bitsloth_save():
        bitsloth_save_pretrained_torchao(
            inner_model,
            transformer_dir,
            tokenizer = tokenizer,
    ...
    try:
        FastSentenceTransformer._add_bitsloth_branding(save_directory)
    except:
        pass
...
def _save_pretrained_gguf(
    ...
    token = None,
    max_shard_size = "5GB",
    temporary_location = "_bitsloth_temporary_saved_buffers",
    ...
    **kwargs,
    ...
    # If it's a PEFT model, bitsloth_save_pretrained_gguf handles merging,
    ...
    # This is where Bitsloth will perform the save + conversion operations
    transformer_dir = os.path.join(save_directory, transformer_path)
    ...
    # 4. Patch environment to ensure Bitsloth treats this embedding model correctly
    @contextlib.contextmanager
    def patch_bitsloth_gguf_save():
    ...
    # 5. Call Bitsloth's GGUF saver on the inner model targeting the transformer subdirectory
    with patch_bitsloth_gguf_save():
    ...
    if push_to_hub:
        ...
        print(f"Bitsloth: Uploading to {repo_id}...")
        try:
            api.create_repo(
            ...
            print(f"Bitsloth: Uploaded to https://huggingface.co/{repo_id}")
        except Exception as e:
            print(f"Bitsloth: Upload failed: {e}")
...
def _push_to_hub_gguf(
    ...
    token = None,
    private = None,
    commit_message = "Upload GGUF SentenceTransformer model trained with Bitsloth",
    commit_description = "Upload GGUF model trained with Bitsloth 2x faster",
    max_shard_size = "5GB",
    temporary_location = "_bitsloth_temporary_saved_buffers",
    maximum_memory_usage = 0.85,
    create_pr = False,
    ...
    **kwargs,
    ...
    try:
        ...
        print(f"Bitsloth Warning: Could not create repo: {e}")

    # Save to temporary directory first
    with tempfile.TemporaryDirectory(prefix = "bitsloth_st_gguf_") as temp_dir:
        print(f"Bitsloth: Converting SentenceTransformer to GGUF format...")
        ...
        print(f"Bitsloth: Uploading GGUF to https://huggingface.co/{full_repo_id}...")
...
This sentence-transformers model was finetuned and converted to GGUF format using [Bitsloth](https://github.com/unslothai/bitsloth).
...
"""
    ...
    # Add tags
    all_tags = ["gguf", "llama-cpp", "bitsloth", "sentence-transformers"]
    if is_vlm:
...
class FastSentenceTransformer(FastModel):
    ...
    def _patch_distilbert_v5():
        ...
        def forward(
            ...
            **kwargs,
            ...
            # patch here: bitsloth gradient checkpointing hook needs positional arguments
            return self.transformer(
                embeddings,
                attention_mask,
                **kwargs,
    ...
    @staticmethod
    def _add_bitsloth_tags(repo_id, token, tags = None):
        """
        Add Bitsloth and sentence-transformers tags to the Hugging Face Hub repository.
        """
        ...
        if tags is None:
            tags = []
        tags.extend(["bitsloth", "sentence-transformers"])
        try:
            api.add_tags(
                repo_id = repo_id,
                tags = tags,
    ...
    @staticmethod
    def _add_bitsloth_branding(save_directory):
        """
        Add Bitsloth branding to the README.md file generated by sentence-transformers.
        """
        readme_path = os.path.join(save_directory, "README.md")
        ...
        # add bitsloth tag to frontmatter
        if "---\ntags:\n" in content:
            ...
            if re.search(pattern, content, re.MULTILINE):
                content = re.sub(
                    pattern, r"\1- bitsloth\n", content, count = 1, flags = re.MULTILINE
                )

        # add branding badge and text
        branding = (
            "\n\nThis model was finetuned with [Bitsloth](https://github.com/unslothai/bitsloth).\n\n"
            '[<img src="https://raw.githubusercontent.com/unslothai/bitsloth/main/images/bitsloth%20made%20with%20love.png" width="200"/>](https://github.com/unslothai/bitsloth)\n'
        )
    ...
    def _load_modules(
                ...
                if class_ref == "sentence_transformers.models.Transformer":
                    ...
                    if os.path.isdir(model_name):
                        ...
                        try:
                            ...
                            print(
                                f"Bitsloth Warning: Could not download module {module_path}: {e}"
                            )
                            continue
                    ...
                    try:
                        ...
                        print(
                            f"Bitsloth Warning: Failed to load module {name} ({class_ref}): {e}"
                        )
        ...
        print(
            "Bitsloth: No modules.json found, falling back to [Transformer, Pooling, Normalize]. This may or may not work."
        )
    ...
    # Encoder model types that benefit from native torch.compile instead of Bitsloth patching
    ENCODER_MODEL_TYPES = {
        "mpnet",
        "bert",
        "distilbert",
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ImportError(
                "Bitsloth: To use `FastSentenceTransformer`, you must install `sentence-transformers`.\n"
                "Run `pip install sentence-transformers` to install it."
            )

        # if for_inference == True, skip Bitsloth optimizations to avoid torch compile issues
        if for_inference:
        ...
        if full_finetuning and (load_in_4bit or load_in_8bit):
            print(
                "Bitsloth: You selected full finetuning support, but 4bit / 8bit is enabled - disabling LoRA / QLoRA."
            )
            load_in_4bit = False
            load_in_8bit = False
            load_in_fp8 = False
        ...
        if int(load_in_4bit) + int(load_in_8bit) + int(load_in_16bit) >= 2:
            raise RuntimeError(
                "Bitsloth: Can only load in 4bit or 8bit or 16bit, not a combination!\n"
        ...
        # This bypasses Bitsloth's auto-compiler which adds @torch.compiler.disable decorators
        # that interfere with torch.compile and cause runtime errors for encoder models.
        # NOTE: The old Bitsloth path is BROKEN for encoder models with torch 2.9+ due to
        # conflicting @torch.compile and @torch.compiler.disable decorators.
        # Set BITSLOTH_COMPILE_DISABLE=1 to disable torch.compile and use the old path.
        is_encoder_model = (
            model_type.lower() in FastSentenceTransformer.ENCODER_MODEL_TYPES
        )
        use_fast_encoder = os.environ.get("BITSLOTH_COMPILE_DISABLE", "0") != "1"
        if use_fast_encoder and is_encoder_model:
            ...
            if dtype is None:
                ...
                print(
                    "Bitsloth: Device does not support bfloat16. Using float16 instead."
                )
                dtype = torch.float16
            ...
            if load_in_4bit:
                print(
                    f"Bitsloth: Using fast encoder path for {model_type} with 4-bit quantization{sdpa_str}"
                )
            else:
                print(
                    f"Bitsloth: Using fast encoder path for {model_type} (torch.compile{sdpa_str})"
                )
            ...
            if _use_gc and _use_gc != False:
                print(
                    "Bitsloth Warning: Gradient checkpointing is incompatible with torch.compile."
                )
            ...
            # Load via native SentenceTransformer (bypasses Bitsloth patching)
            ...
            st_model._bitsloth_fast_encoder = True
            ...
            def _push_to_hub_merged(self, repo_id, **push_kwargs):
                ...
                try:
                    ...
                    pass
                FastSentenceTransformer._add_bitsloth_tags(repo_id, hub_token)
                ...
                print(f"Bitsloth: Pushed to https://huggingface.co/{repo_id}")
        ...
        if is_encoder_model and load_in_4bit:
            print(
                "Bitsloth Warning: 4-bit quantization adds ~2.3x overhead for encoder models."
            )
        ...
        if fp8:
            logging.info("Bitsloth: Disabling fp8 for model")
        load_in_fp8 = False

        # this is a fix for Snowflake/snowflake-arctic-embed-l-v2.0
        # it has pooler weights which we don't care about for training,
        # however bitsloth throws an exception if "BITSLOTH_WARN_UNINITIALIZED" == 1 and it sees unused weights
        old_environ = os.environ.get("BITSLOTH_WARN_UNINITIALIZED", "1")
        ...
        if not has_modules_json and load_in_4bit:
            print(
                "Bitsloth: No modules.json found. This is not a sentence-transformers model.\n"
        ...
        try:
            model, tokenizer = FastModel.from_pretrained(
                ...
                bitsloth_tiled_mlp = bitsloth_tiled_mlp,
                **kwargs,
            )
        finally:
            os.environ["BITSLOTH_WARN_UNINITIALIZED"] = old_environ
        ...
        def _save_pretrained_merged(self, save_directory, **kwargs):
            ...
            if self.no_modules:
                ...
                print(
                    "Bitsloth: No modules detected. Using standard merge_and_unload for saving..."
                )
                safe_kwargs = kwargs.copy()
                # filter out Bitsloth-specific args that are not in huggingface's save_pretrained
                bitsloth_args = [
                    "save_method",
                    "temporary_location",
                    "maximum_memory_usage",
                ]
                for k in bitsloth_args:
            ...
            # add Bitsloth branding to the generated README
            try:
                FastSentenceTransformer._add_bitsloth_branding(save_directory)
            except Exception as e:
                print(f"Bitsloth Warning: Failed to add branding to README: {e}")
        ...
        def _push_to_hub_merged(self, repo_id, **kwargs):
            ...
            FastSentenceTransformer._add_bitsloth_tags(repo_id, token)
            ...
            print(
                f"Bitsloth: Successfully pushed merged model to https://huggingface.co/{repo_id}"
            )
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        if isinstance(model, SentenceTransformer):
            # Check if this is a fast encoder model (uses torch.compile instead of Bitsloth patching)
            is_fast_encoder = getattr(model, "_bitsloth_fast_encoder", False)
            ...
            if is_fast_encoder:
                ...
                if is_quantized:
                    ...
                    try:
                        ...
                        print("Bitsloth: Prepared quantized model for k-bit training")
                        ...
                        if "does not support gradient checkpointing" in str(e):
                            ...
                            print(
                                f"Bitsloth Warning: {inner_model.__class__.__name__} does not support gradient checkpointing. Skipping."
                            )
                            inner_model = prepare_model_for_kbit_training(
                                inner_model,
                            ...
                            print(
                                "Bitsloth: Prepared quantized model for k-bit training (without gradient checkpointing)"
                            )
                        else:
                            raise
                ...
                # Enable gradient checkpointing if requested (only for non-quantized, since prepare_model handles it)
                    ...
                    if hasattr(inner_model, "gradient_checkpointing_enable"):
                        try:
                            ...
                            print("Bitsloth: Enabled gradient checkpointing")
                            gc_enabled = True
                        except ValueError as e:
                            if "does not support gradient checkpointing" in str(e):
                                print(
                ...
                if compile_mode is None and not gc_enabled:
                    compile_mode = "default"
                    print(
                        "Bitsloth: Re-enabling torch.compile since gradient checkpointing is not supported"
                    )
                ...
                if compile_mode is not None:
                    ...
                    print(
                        f"Bitsloth: torch.compile will be applied automatically if max_steps > {model._compile_threshold}"
                    )
                else:
                    model._compile_mode = None
                    model._compile_pending = False
                    print(
                        "Bitsloth: torch.compile disabled (gradient checkpointing enabled)"
                    )
...
def _patch_sentence_transformer_trainer():
    ...
    if getattr(SentenceTransformerTrainer, "_bitsloth_auto_compile_patched", False):
    ...
    def _patched_init(self, *args, **kwargs):
        ...
        if (
            ...
            if max_steps > 0 and max_steps >= threshold:
                print(
                    f"Bitsloth: Auto-compiling model ({max_steps} steps >= {threshold} threshold)"
                )
                FastSentenceTransformer._apply_torch_compile(model, mode = compile_mode)
                model._compile_pending = False
            elif max_steps > 0:
                print(
                    f"Bitsloth: Skipping torch.compile ({max_steps} steps < {threshold} threshold)"
                )
                model._compile_pending = False
        ...
        if os.environ.get("BITSLOTH_FORCE_FLOAT32", "0") == "1":
            if hasattr(self, "args") and self.args is not None:
                if self.args.fp16 or self.args.bf16:
                    print(
                        "Bitsloth: Switching to float32 training since model cannot work with float16"
                    )
    ...
    SentenceTransformerTrainer._bitsloth_auto_compile_patched = True
