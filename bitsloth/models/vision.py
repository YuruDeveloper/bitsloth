...
def bitsloth_base_fast_generate(
    self,
    *args,
    **kwargs,
):
    if len(args) != 0:
        ...
        if type(kwargs[key]) is not torch.Tensor:
            raise TypeError("Bitsloth: You need to pass in input_ids to .generate!")
    ...
    do_bfloat16_mixed_precision = (
        os.environ.get("BITSLOTH_BFLOAT16_MIXED_PRECISION", "0") == "1"
    )
    ...
    if getattr(
        ...
        if os.environ.get("BITSLOTH_DISABLE_STATIC_GENERATION", "0") == "0":
...
class FastBaseModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if bitsloth_vllm_standby and os.environ.get("BITSLOTH_VLLM_STANDBY", "0") != "1":
            raise RuntimeError(
                "Bitsloth: BITSLOTH_VLLM_STANDBY is True, but BITSLOTH_VLLM_STANDBY is not set to 1!"
            )

        if model_types is None:
            raise RuntimeError(
                "Bitsloth: Please use FastModel or FastVisionModel and not use FastBaseModel directly!"
            )
        if os.environ.get("BITSLOTH_MODEL_NAME", "") == "":
            os.environ["BITSLOTH_MODEL_NAME"] = model_name.lower()
        ...
        if is_vlm and fast_inference:
            if not any(arch in VLLM_SUPPORTED_VLM for arch in model_types):
                raise RuntimeError(
                    f"Bitsloth: Fast inference is only supported for Language models and Qwen2.5-VL, Gemma3 among vision models. "
        ...
        os.environ["BITSLOTH_USE_NEW_MODEL"] = "1"
        ...
        statistics = (
            f"==((====))==  Bitsloth {__version__}: Fast {arch_name} patching. Transformers: {transformers_version}.{vllm_version}\n"
        ...
        elif os.environ.get("BITSLOTH_FORCE_FLOAT32", "0") == "1":
        ...
        if os.environ.get("BITSLOTH_FORCE_FLOAT32", "0") == "1":
            print(
                f"Bitsloth: Using float16 precision for {model_type_arch} won't work! Using float32."
            )
        ...
        if os.environ.get("BITSLOTH_FORCE_CUSTOM_DTYPE", "") != "":
            ...
            allow_float16_runs = (
                ...
                dtype == torch.float16
                or os.environ.get("BITSLOTH_FORCE_FLOAT32", "0") == "1"
            )
        ...
        if not supports_sdpa and kwargs.get("attn_implementation") == "sdpa":
            if os.environ.get("BITSLOTH_ENABLE_FLEX_ATTENTION", "0") == "0":
                print(
                    f"Bitsloth: {model_type_arch.title()} does not support SDPA - switching to fast eager."
                )
        ...
        if load_in_4bit:
            ...
            print(
                "Bitsloth: QLoRA and full finetuning all not selected. Switching to 16bit LoRA."
            )

        if full_finetuning:
            os.environ["BITSLOTH_ENABLE_FULL_FINETUNING"] = "1"
            if dtype == torch.bfloat16:
                if float32_mixed_precision != True:
                    print(
                        f"Bitsloth: Using bfloat16 full finetuning which cuts memory usage by 50%.\n"
                        f"To enable float32 training, use `float32_mixed_precision = True` during FastLanguageModel.from_pretrained"
                    )
                else:
                    print(
                        f"Bitsloth: Using full float32 full finetuning. "
                    ...
                    os.environ["BITSLOTH_BFLOAT16_MIXED_PRECISION"] = "1"
            else:
                print(
                    "Bitsloth: Float16 full finetuning uses more memory since we upcast weights to float32."
                )
        else:
            os.environ["BITSLOTH_ENABLE_FULL_FINETUNING"] = "0"
            ...
            if hasattr(auto_config, "quantization_config"):
                ...
                if (
                    ...
                    if "bitsandbytes_4bit" not in AUTO_QUANTIZATION_CONFIG_MAPPING:
                        raise KeyError(
                            "Bitsloth: AUTO_QUANTIZATION_CONFIG_MAPPING does not have `bitsandbytes_4bit`"
                        )
        ...
        if not fast_inference:
            ...
            if offload_embedding:
                if bool(
                    ...
                    print(f"Bitsloth: Offloading embeddings to RAM to save {ngb} GB.")
                    embed_tokens.to("cpu")
            ...
            if full_finetuning:
                ...
                raise NotImplementedError(
                    "Bitsloth: `fast_inference=True` cannot be used together with `full_finetuning=True`.\n"
        ...
        if os.environ.get("BITSLOTH_HIGH_PRECISION_LAYERNORM", "0") == "1":
        ...
        # Fix _Bitsloth_Patched_ prefix in local config files from old saves (issue #4085)
        ...
        if (tokenizer is None or _processor_is_degraded) and is_vlm:
            ...
            if tokenizer is None:
                ...
                print(
                    f"Bitsloth: Warning - VLM processor fallback returned None for model_type={model_type_arch}",
                    file = sys.stderr,
                )
        ...
        if tokenizer is None:
            ...
            try:
                ...
                try:
                    ...
                    del model
                    raise RuntimeError(
                        "Bitsloth: The tokenizer is weirdly not loaded? Please check if there is one."
                    )
        ...
        # Patch generate
        if os.environ.get("BITSLOTH_DISABLE_FAST_GENERATION", "0") == "0" and hasattr(
            model, "generate"
        ):
            if model.generate.__name__ != "bitsloth_base_fast_generate":
                model._old_generate = model.generate
                bitsloth_base_fast_generate.__doc__ = model._old_generate.__doc__
                model.generate = types.MethodType(bitsloth_base_fast_generate, model)
        model._bitsloth_trust_remote_code = trust_remote_code
        # Post patches
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        if isinstance(model, PeftModelForCausalLM):
            raise RuntimeError(
                "Bitsloth: You already added LoRA adapters to your model!"
            )
        ...
        if hasattr(model, "vllm_engine"):
            if (
                ...
                raise RuntimeError("Bitsloth: LoRA is not enabled for this model!")
            if finetune_vision_layers:
                ...
                raise RuntimeError(
                    "Bitsloth: Finetuning vision layers is not supported for fast_inference. Only text layers are supported!"
                )
            if model.config.model_type in VLLM_NON_LORA_VLM:
                ...
                # TODO: Update this once vLLM V1 supports Llama 3.2 aka mllama
                raise RuntimeError(
                    "Bitsloth: LoRA finetuning for Llama 3.2 aka mllama models is not supported with fast_inference!"
                )
        ...
        trust_remote_code = getattr(model, "_bitsloth_trust_remote_code", False)
        model = FastBaseModel.post_patch_model(
            model,
            use_gradient_checkpointing = use_gradient_checkpointing,
            trust_remote_code = trust_remote_code,
